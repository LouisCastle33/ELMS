...
// Only show requests AFTER HR approves
$filter_sql = "WHERE hr_status='Approved'";
if (!empty($search)) {
...
// Counts (Dept Head only works on HR-approved requests)
$pending_count = $conn->query("SELECT COUNT(*) AS c 
    FROM leave_applications 
    WHERE hr_status='Approved' AND dept_head_status='Pending'")
    ->fetch_assoc()['c'];

$approved_count = $conn->query("SELECT COUNT(*) AS c 
    FROM leave_applications 
    WHERE hr_status='Approved' AND dept_head_status='Approved'")
    ->fetch_assoc()['c'];

$rejected_count = $conn->query("SELECT COUNT(*) AS c 
    FROM leave_applications 
    WHERE hr_status='Approved' AND dept_head_status='Rejected'")
    ->fetch_assoc()['c'];
...
<section id="content">
    <nav>
        ...
        <a href="#" class="nav-link">Dept Head Dashboard</a>
    </nav>
    ...
    <main>
        ...
        <div class="filter-container" style="margin: 20px 0;">
            <form method="GET" style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
                ...
                <a href="dept_dashboard.php" style="color:#555;">Reset</a>
            </form>
        </div>
